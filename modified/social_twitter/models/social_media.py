...
class SocialMediaTwitter(models.Model):
    ...
    def _action_add_account(self):
        ...
        if not twitter_consumer_key or not twitter_consumer_secret_key:
            raise UserError(_("Please configure your X/Twitter Consumer Key and Secret Key in Settings > Social Marketing."))
    ...
    def _get_twitter_oauth_signature(self, method, url, params, oauth_token_secret=''):
        ...
        twitter_consumer_secret_key = self.env['ir.config_parameter'].sudo().get_param('social.twitter_consumer_secret_key')
        if not twitter_consumer_secret_key:
            raise UserError(_("Twitter Consumer Secret Key is not configured."))
