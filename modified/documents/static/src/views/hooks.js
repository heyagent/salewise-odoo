...
// Removed web_enterprise dependency - was only used for promotional dialog
...
export function useDocumentView(helpers) {
    ...
    const _openAutomations = async ({ folderId, folderDisplayName }) => {
        ...
        if (!checkBaseAutomation > 0) {
            // Show notification instead of promotional dialog (web_enterprise dependency removed)
            return notification.add(_t("Install the Base Automation module to create automation rules."), {
                title: _t("Automation Module Required"),
                type: "info",
            });
        }
